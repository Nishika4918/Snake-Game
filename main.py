...
class Apple:
    def __init__(self, parent_screen):
        self.image = pygame.image.load("extra/apple.jpg").convert()
...
class Snake:
    def __init__(self, parent_screen, length):
        self.length = length
        self.parent_screen = parent_screen
        self.block = pygame.image.load("extra/block.jpg").convert()
        self.x = [40]*length
        self.y = [40]*length
        self.direction = 'down'
...
class Game:
    ...
    def is_collision(self, x1, y1, x2, y2):
        if x1 >= x2 and x1 < x2 + SIZE:
            if y1 >= y1 and y1 < y2 + SIZE:
                return True
        return False

    def play_background_music(self):
        pygame.mixer.music.load('extra/bg_music_1.mp3')
        pygame.mixer.music.play(-1, 0)

    def play_sound(self, sound):
        sound = pygame.mixer.Sound(f'extra/{sound}.mp3')
        pygame.mixer.Sound.play(sound)

    def render_background(self):
        bg = pygame.image.load('extra/background.jpg')
